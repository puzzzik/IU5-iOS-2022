...
final class Lab5StuffViewController: UIViewController {
	...
	var output: Lab5StuffViewOutput!
	...
	private func setupActionButton() {
		...
		actionButton.addTarget(self, action: #selector(userDidTouchButton), for: .touchUpInside)
	}
	
	private func setupMySwitch() {
		mySwitch.isOn = false
		mySwitch.preferredStyle = .sliding
		mySwitch.addTarget(self, action: #selector(userDidChangeSwitch), for: .valueChanged)
	}
	...
	private func setupTextField() {
		...
		textField.addTarget(self, action: #selector(userDidChangeTextField), for: .editingDidBegin)
	}
	...
	private func setupSegmentControl() {
		...
		segmentControl.addTarget(self, action: #selector(userDidChangeSegmentControl), for: .valueChanged)
	}
	...
	private func setupSlider() {
		...
		slider.addTarget(self, action: #selector(userDidChangeSlider), for: .valueChanged)
	}
	
	@objc private func userDidChangeSwitch() {
		output.userDidChangeSwitch(label: mySwitchLabel, switcher: mySwitch, textField: textField)
	}
	
	@objc private func userDidChangeSlider() {
		output.userDidChangeSlider(label: sliderLabel, slider: slider)
	}
	
	@objc private func userDidChangeTextField() {
		output.userDidChangeTextField(label: textFieldLabel, switcher: mySwitch)
	}
	
	@objc private func userDidChangeSegmentControl() {
		output.userDidChangeSegmentControl(label: segmentControlLabel, segmentControl: segmentControl)
	}
	
	@objc private func userDidTouchButton() {
//		let actionSheetController = output.userDidTouchButton(label: actionButtonLabel)
		guard let actionSheetController = output.userDidTouchButton(label: actionButtonLabel) else {
			return
		}
